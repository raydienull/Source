...
bool CClient::OnTarg_Obj_Set( CObjBase * pObj )
{
	...
	if ( pObj->IsItem() )
	{
		...
		if ( pItem->GetAmount() > 1 )
			sprintf(pszLogMsg, "'%s' commands uid=0%x (%s) [amount=%u] to '%s'", static_cast<LPCTSTR>(GetName()), static_cast<DWORD>(pObj->GetUID()), static_cast<LPCTSTR>(pObj->GetName()), pItem->GetAmount(), static_cast<LPCTSTR>(m_Targ_Text));
		else
			sprintf(pszLogMsg, "'%s' commands uid=0%x (%s) to '%s'", static_cast<LPCTSTR>(GetName()), static_cast<DWORD>(pObj->GetUID()), static_cast<LPCTSTR>(pObj->GetName()), static_cast<LPCTSTR>(m_Targ_Text));
	}
	else
		sprintf(pszLogMsg, "'%s' commands uid=0%x (%s) to '%s'", static_cast<LPCTSTR>(GetName()), static_cast<DWORD>(pObj->GetUID()), static_cast<LPCTSTR>(pObj->GetName()), static_cast<LPCTSTR>(m_Targ_Text));
...
int CClient::OnSkill_EvalInt( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	if ( iIntEntry < 0 )
		iIntEntry = 0;
	if ( iIntEntry >= static_cast<int>(COUNTOF( sm_szIntDesc )))
	...
	if ( iSkillLevel > 400 )	// magery skill and mana level ?
	{
		...
		if ( iMagicEntry < 0 )
			iMagicEntry = 0;
		if ( iMagicEntry >= static_cast<int>(COUNTOF(sm_szMagicDesc)))
		...
		if ( iManaEntry < 0 )
			iManaEntry = 0;
		if ( iManaEntry >= static_cast<int>(COUNTOF(sm_szManaDesc)))
...
int CClient::OnSkill_Anatomy( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	if ( iStrEntry < 0 )
		iStrEntry = 0;
	if ( iStrEntry >= static_cast<int>(COUNTOF( sm_szStrEval )))
	...
	if ( iDexEntry < 0 )
		iDexEntry = 0;
	if ( iDexEntry >= static_cast<int>(COUNTOF( sm_szDexEval )))
...
static LPCTSTR const sm_Txt_LoomUse[] =
{
	...
	g_Cfg.GetDefaultMsg( DEFMSG_ITEMUSE_BOLT_5 )
		...
		if ( (iHave + iUsed) < static_cast<int>(COUNTOF( sm_Txt_LoomUse ) - 1) )
		{
